...
roc_curve_default_rf <- default_roc_data_rf %>%
  ...
  theme_light() +
  labs(
    title = "Random Forest unbalanced ROC Curve",
...
print(roc_curve_default_rf)
print(roc_curve_final_rf)

```

```{r}
final_rf_smote <- function(ntrees, max_depth, min_rows, mtries, sample_rate) {
  message("Evaluating with parameters: ntrees = ", ntrees,
          ", max_depth = ", max_depth,
          ", min_rows = ", min_rows,
          ", mtries = ", mtries,
...
```{r}
bounds_rf_smote <- list(
  ntrees = c(50, 200),
  max_depth = c(5, 50),
  min_rows = c(1, 20),
  mtries = c(1, min(length(x1) - 1, 20)),  # Ensure mtries is in the valid range [1, total_features - 1] and not too large
  sample_rate = c(0.5, 1.0)
)
```
...
```{r}
tic()
opt_results_rf_smote <- BayesianOptimization(
  FUN = final_rf_smote,
  bounds = bounds_rf_smote,
  init_points = 12,
  n_iter = 20,
  acq = "ucb"
)
...
print(opt_results_rf_smote$History)

best_result_index_rf_smote <- which.min(opt_results_rf_smote$History$Value)
best_result_rf_smote <- opt_results_rf_smote$History[best_result_index_rf_smote, ]

cat("\nBest Parameters (Highest Accuracy) for SMOTE:\n")
print(best_result_rf_smote)

cat("\nBest Accuracy for SMOTE:\n")
print(-best_result_rf_smote$Value)

opt_results_rf_smote$Best_Par <- best_result_rf_smote[, c("ntrees", "max_depth", "min_rows",  "sample_rate")]
...
```{r}
tuned_rf_smote <- h2o.randomForest(
  y = y1,
  training_frame = smote_trn_h2o,
  validation_frame = smote_dev_h2o,
...
```{r}
tuned_predictions_rf_smote <- h2o.predict(tuned_rf_smote, tst_usfs_h2o)
tuned_metrics_rf_smote <- h2o.performance(tuned_rf_smote, tst_usfs_h2o)
```

```{r}
tuned_confusion_matrix_rf_smote <- h2o.confusionMatrix(tuned_metrics_rf_smote, thresholds = threshold)
tuned_accuracy_rf_smote <- h2o.accuracy(tuned_metrics_rf_smote, thresholds = threshold)[[1]]
tuned_precision_rf_smote <- h2o.precision(tuned_metrics_rf_smote, thresholds = threshold)[[1]]
tuned_recall_rf_smote <- h2o.recall(tuned_metrics_rf_smote, thresholds = threshold)[[1]]
tuned_auc_rf_smote <- h2o.auc(tuned_metrics_rf_smote)
tuned_mcc_rf_smote <- h2o.mcc(tuned_metrics_rf_smote, thresholds = threshold)[[1]]

final_fpr_rf_smote <- as.vector(h2o.fpr(tuned_metrics_rf_smote)[, "fpr"])
final_tpr_rf_smote <- as.vector(h2o.tpr(tuned_metrics_rf_smote)[, "tpr"])
final_roc_data_rf_smote <- data.frame(fpr = final_fpr_rf_smote, tpr = final_tpr_rf_smote)
```

```{r}
final_metrics_vector_rf_smote <- c(
  Accuracy = tuned_accuracy_rf_smote,
  Precision = tuned_precision_rf_smote,
  Recall = tuned_recall_rf_smote,
  AUC = tuned_auc_rf_smote,
  MCC = tuned_mcc_rf_smote
)

print("Tuned RF Model Metrics (SMOTE):")
print(final_metrics_vector_rf_smote)
print(tuned_confusion_matrix_rf_smote)
```

```{r}
roc_curve_final_rf_smote <- final_roc_data_rf_smote %>%
  ggplot(aes(x = fpr, y = tpr)) +
  ...
  theme_light() +
  labs(
    title = "Tuned RF SMOTE ROC Curve",
    x = "False Positive Rate (1 - Specificity)",
    y = "True Positive Rate (Sensitivity)"
  )
print(roc_curve_final_rf_smote)
