...
roc_curve_final_rf <- final_roc_data_rf %>%
  ...
  theme_light() +
  labs(
    title = "Random Forest + IQR ROC Curve",
...
```{r}
trn_h2o2 <- as.h2o(trn_usfs2)
dev_h2o2 <- as.h2o(dev_usfs2)
...
final_rf2 <- function(ntrees, max_depth, min_rows, mtries, sample_rate) {
  message("Evaluating with parameters: ntrees = ", ntrees,
          ", max_depth = ", max_depth,
          ", min_rows = ", min_rows,
          ", mtries = ", mtries,
...
bounds_rf2 <- list(
  ...
  max_depth = c(5, 50),
  min_rows = c(1, 20),
  mtries = c(1, min(length(x2) - 1, 20)),  # Ensure mtries is in the valid range [1, total_features - 1] and not too large
  sample_rate = c(0.5, 1.0))
...
```{r bayesian optimization function}
tic()
opt_results_rf2 <- BayesianOptimization(
  FUN = final_rf2,
  bounds = bounds_rf2,
  init_points = 12,
  n_iter = 10,
...
print(opt_results_rf2$History)

best_result_index_rf2 <- which.min(opt_results_rf2$History$Value)
best_result_rf2 <- opt_results_rf2$History[best_result_index_rf2, ]

cat("\nBest Parameters (Highest Accuracy) for New Data:\n")
print(best_result_rf2)

cat("\nBest Accuracy for New Data:\n")
print(-best_result_rf2$Value)

opt_results_rf2$Best_Par <- best_result_rf2[, c("ntrees", "max_depth", "min_rows",  "sample_rate")]
...
```{r predicting tuned rf}
tuned_predictions_rf2 <- h2o.predict(tuned_rf2, tst_usfs2)
tuned_metrics_rf2 <- h2o.performance(tuned_rf2, tst_usfs2)
```

```{r eval metrics tuned rf}
tuned_confusion_matrix_rf2 <- h2o.confusionMatrix(tuned_metrics_rf2, thresholds = threshold)
tuned_accuracy_rf2 <- h2o.accuracy(tuned_metrics_rf2, thresholds = threshold)[[1]]
tuned_precision_rf2 <- h2o.precision(tuned_metrics_rf2, thresholds = threshold)[[1]]
tuned_recall_rf2 <- h2o.recall(tuned_metrics_rf2, thresholds = threshold)[[1]]
tuned_auc_rf2 <- h2o.auc(tuned_metrics_rf2)
tuned_mcc_rf2 <- h2o.mcc(tuned_metrics_rf2, thresholds = threshold)[[1]]

final_fpr_rf2 <- as.vector(h2o.fpr(tuned_metrics_rf2)[, "fpr"])
final_tpr_rf2 <- as.vector(h2o.tpr(tuned_metrics_rf2)[, "tpr"])
final_roc_data_rf2 <- data.frame(fpr = final_fpr_rf2, tpr = final_tpr_rf2)
```

```{r printing eval metric tuned rf}
final_metrics_vector_rf2 <- c(
  Accuracy = tuned_accuracy_rf2,
  Precision = tuned_precision_rf2,
  Recall = tuned_recall_rf2,
  AUC = tuned_auc_rf2,
  MCC = tuned_mcc_rf2)

print("Tuned RF Model Metrics for New Data:")
print(final_metrics_vector_rf2)
print(tuned_confusion_matrix_rf2)
```

```{r}
roc_curve_final_rf2 <- final_roc_data_rf2 %>%
  ggplot(aes(x = fpr, y = tpr)) +
  ...
  theme_light() +
  labs(
    title = "Random Forest + Mahalanobis removal ROC curve",
    x = "False Positive Rate (1 - Specificity)",
    y = "True Positive Rate (Sensitivity)")
print(roc_curve_final_rf2)
